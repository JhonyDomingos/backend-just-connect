...
class DeleteTagController {
  ...
  async delete(req: Request, res: Response): Promise<Response> {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "Tag deletada com sucesso", deletedTag }); //Retorna uma mensagem de sucesso e a tag deletada
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Erro ao deletar tag";
      return res.status(400).json({ error: errorMessage }); // Retorna um erro caso a deleção falhe
    }
  }
}
